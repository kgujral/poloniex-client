...
public class PoloniexLendingService implements LendingService {
  ...
  public List<PoloniexLendingHistory> returnLendingHistory(int hours, int limit) {
    ...
    try {
      ...
      LOG.trace("Retrieved and mapped {} {} {} lendingHistory in {} ms", lendingHistory.size(), hours, limit,
   ...
   */
  @Override
  public PoloniexLendingResult createLoanOffer(String currency, BigDecimal amount, BigDecimal lendingRate,
      int duration, boolean autoRenew) {
    long start = System.currentTimeMillis();
    ...
    try {
      ...
      LogManager.getLogger(PoloniexLendingService.class).trace(
          "Executed and mapped createLoanOffer for {} {} {} {} {} in {} ms", currency, amount.toPlainString(),
          lendingRate.toPlainString(), duration, autoRenew ? 1 : 0, System.currentTimeMillis() - start);
    } catch (Exception ex) {
      LogManager.getLogger(PoloniexLendingService.class).error(
          "Error executing createLoanOffer for {} {} {} {} {} - {}", currency, amount.toPlainString(),
          lendingRate.toPlainString(), duration, autoRenew ? 1 : 0, ex.getMessage());
    }
  ...
  public PoloniexLendingResult cancelLoanOffer(String orderNumber) {
    ...
    try {
      ...
      result = mapper.mapLendingResult(res);
      LogManager.getLogger(PoloniexLendingService.class).trace("Executed and mapped cancelLoanOffer for {} in {} ms",
          orderNumber, System.currentTimeMillis() - start);
    } catch (Exception ex) {
      LogManager.getLogger(PoloniexLendingService.class).error("Error executing cancelLoanOffer for {} - {}",
  ...
  public List<PoloniexLoanOffer> returnOpenLoanOffers(String currency) {
    ...
    try {
      ...
      LOG.trace("Retrieved and mapped {} {} OpenLoanOffers in {} ms", currency, offers.size(),
          System.currentTimeMillis() - start);
      return offers;
    } catch (Exception ex) {
  ...
  public PoloniexLendingResult toggleAutoRenew(String orderNumber) {
    ...
    try {
      ...
      result = mapper.mapLendingResult(res);
      LogManager.getLogger(PoloniexLendingService.class).trace("Executed and mapped toggleAutoRenew for {} in {} ms",
          orderNumber, System.currentTimeMillis() - start);
    } catch (Exception ex) {
      LogManager.getLogger(PoloniexLendingService.class).error("Error executing toggleAutoRenew for {} - {}",
          orderNumber, ex.getMessage());
    }
