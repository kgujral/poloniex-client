...
import com.cf.data.model.poloniex.PoloniexOrderBook;
...
public class PoloniexExchangeService implements ExchangeService {
  ...
  public PoloniexExchangeService(PriceDataAPIClient publicClient, TradingAPIClient tradingClient,
      PoloniexDataMapper mapper) {
   ...
   * @param periodInSeconds The candlestick chart data period. Valid values are 300 (5 min), 900 (15
   *        minutes), 7200 (2 hours), 14400 (4 hours), 86400 (daily)
   * @param startEpochInSeconds UNIX timestamp format and used to specify the start date of the data
   *        returned
  ...
  /**
   * * If you are enrolled in the maker-taker fee schedule, returns your current trading fees and
   * trailing 30-day volume in BTC. This information is updated once every 24 hours.
   *
  ...
  public List<PoloniexOpenOrder> returnOpenOrders(String currencyPair) {
    ...
    try {
      ...
      LOG.trace("Retrieved and mapped {} {} open orders in {} ms", openOrders.size(), currencyPair,
   ...
   * @param immediateOrCancel Order can be partially or completely filled, but any portion of the
   *        order that cannot be filled immediately will be canceled rather than left on the order
   *        book
   * @param postOnly A post-only order will only be placed if no portion of it fills immediately;
   *        this guarantees you will never pay the taker fee on any part of the order that fills
   * @return PoloniexOrderResult
   */
  @Override
  public PoloniexOrderResult sell(String currencyPair, BigDecimal sellPrice, BigDecimal amount, boolean fillOrKill,
      boolean immediateOrCancel, boolean postOnly) {
    long start = System.currentTimeMillis();
    PoloniexOrderResult orderResult = null;
    try {
      String sellTradeResult =
          tradingClient.sell(currencyPair, sellPrice, amount, fillOrKill, immediateOrCancel, postOnly);
      orderResult = mapper.mapTradeOrder(sellTradeResult);
      LOG.trace("Executed and mapped {} sell order {} in {} ms", currencyPair, sellTradeResult,
          System.currentTimeMillis() - start);
    } catch (Exception ex) {
   ...
   */
  @Override
  public PoloniexOrderResult buy(String currencyPair, BigDecimal buyPrice, BigDecimal amount, boolean fillOrKill,
      boolean immediateOrCancel, boolean postOnly) {
    long start = System.currentTimeMillis();
    PoloniexOrderResult orderResult = null;
    try {
      String buyTradeResult =
          tradingClient.buy(currencyPair, buyPrice, amount, fillOrKill, immediateOrCancel, postOnly);
      orderResult = mapper.mapTradeOrder(buyTradeResult);
      LOG.trace("Executed and mapped {} buy order {} in {} ms", currencyPair, buyTradeResult,
          System.currentTimeMillis() - start);
    } catch (Exception ex) {
  ...
  public PoloniexOrderResult moveOrder(String orderNumber, BigDecimal rate, Boolean immediateOrCancel, Boolean postOnly) {
    ...
    try {
      ...
      LogManager.getLogger(PoloniexExchangeService.class).trace("Executed and mapped move order for {} in {} ms",
          orderNumber, System.currentTimeMillis() - start);
    } catch (Exception ex) {
      LogManager.getLogger(PoloniexExchangeService.class).error("Error executing move order for {} - {}", orderNumber,
          ex.getMessage());
    }
  ...
  public PoloniexOrderBook returnOrderBook(String currencyPair, int depth) {
    ...
    try {
      ...
      LOG.error("Error retrieving orderBookData for {} - {}", currencyPair, ex.getMessage());
    }
