...
public class PoloniexPublicAPIClient implements PriceDataAPIClient {
  ...
  public String returnTicker() {
    try {
      ...
      LogManager.getLogger(PoloniexPublicAPIClient.class).warn(
          "Call to return ticker API resulted in exception - " + ex.getMessage(), ex);
    }
  ...
  public String getChartData(String currencyPair, Long periodInSeconds, Long startEpochSeconds, Long endEpochSeconds) {
    return getChartData(currencyPair, startEpochSeconds.toString(), endEpochSeconds.toString(),
  ...
  private String getChartData(String currencyPair, String startEpochInSec, String endEpochInSec, String periodInSec) {
    try {
      String url =
          PUBLIC_URL + "command=returnChartData&currencyPair=" + currencyPair + "&start=" + startEpochInSec + "&end="
              + endEpochInSec + "&period=" + periodInSec;
      return client.getHttp(url, null);
    } catch (IOException ex) {
      LogManager.getLogger(PoloniexPublicAPIClient.class).warn(
          "Call to Chart Data API resulted in exception - " + ex.getMessage(), ex);
    }
  ...
  public String orderBook(String currencyPair, int depth) {
    ...
    try {
      String url = PUBLIC_URL + "command=returnOrderBook&currencyPair=" + currencyPair + "&depth=" + depth;
      return client.getHttp(url, null);
    } catch (IOException ex) {
      LogManager.getLogger(PoloniexPublicAPIClient.class).warn(
          "Call to return orderBook API resulted in exception - " + ex.getMessage(), ex);
    }
